...
*/
AP_RangeFinder_SRF02I2C::AP_RangeFinder_SRF02I2C(RangeFinder &_ranger, uint8_t instance, RangeFinder::RangeFinder_State &_state)
    : AP_RangeFinder_Backend(_ranger, instance, _state)
    , _dev(hal.i2c_mgr->get_device(1, AP_RANGE_FINDER_SRF02_DEFAULT_ADDR))
{
}
...
/*
   ...
   there.
*/
AP_RangeFinder_Backend *AP_RangeFinder_SRF02I2C::detect(RangeFinder &_ranger, uint8_t instance,
                                                             RangeFinder::RangeFinder_State &_state)
{
    AP_RangeFinder_SRF02I2C *sensor
        = new AP_RangeFinder_SRF02I2C(_ranger, instance, _state);
    if (!sensor) {
        return nullptr;
    }
...
bool AP_RangeFinder_SRF02I2C::_init(void)
{
    ...
    hal.scheduler->delay(AP_RANGE_FINDER_SRF02_RANGING_TIME);
    ...
    _dev->register_periodic_callback(70000,
                                     FUNCTOR_BIND_MEMBER(&AP_RangeFinder_SRF02I2C::_timer, bool));
...
{
    uint8_t cmd = AP_RANGE_FINDER_SRF02_CMD_TAKE_RANGE_CM  ;
...
*/
bool AP_RangeFinder_SRF02I2C::_timer(void)
{
    uint16_t d;
    if (get_reading(d)) {
        if (_sem->take(0)) {
